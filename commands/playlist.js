...
module.exports = {
	...
	run: async function (client, message, args) {
		...
		if (!permissions.has("CONNECT")) return sendError("Eu não tenho permissões para entrar no canal de voz.", message.channel);
		if (!permissions.has("SPEAK")) return sendError("Eu não consigo falar no canal de voz pois eu não tenho permissões.", message.channel);
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			try {
				...
				return message.channel.send({
					embed: {
						color: "GREEN",
						description: `✅  **|**  Playlist: **\`${videos[0].title}\`**Foi adicionada na fila.`
					}
				})
			} catch (error) {
				console.error(error);
